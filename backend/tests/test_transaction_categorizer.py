...
@patch("routes.transactions.transaction_categorizer", new_callable=AsyncMock)
def test_transaction_categorizer_transfer_happy_path(mock_categorizer, create_two_accounts):
  ...
  accounts = client.get("/accounts", headers = {"Authorization" : f"Bearer {token}"})
  accounts_info = [{"id": a["id"], "name": a["bank_name"], "type": a["account_type"]} for a in accounts.json()]
  response = client.post("/transactions/categorize", headers = {"Authorization" : f"Bearer {token}"}, json = {"description": "Transfered 43.67 to savings account from Chase", "accounts_info": accounts_info})
    
  assert response.status_code == 200
  assert response.json()["account_id"] == 1 
  assert response.json()["destination_account_id"] == 2

@patch("routes.transactions.transaction_categorizer", new_callable=AsyncMock)
def test_transaction_categorizer_bad_transfer(mock_categorizer, create_two_accounts):
  client, token, first_account_id, second_account_id= create_two_accounts 
  mock_categorizer.return_value = {
    "account_id": first_account_id, 
  ...
  assert response.status_code == 422 
  assert response.json()["detail"] == "Destination account name does not exist."
