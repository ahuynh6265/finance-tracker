...
try:
  ...
  for transaction in transactions: 
    account = account_lookup(transaction.account_id, db, 25)

    if transaction.transaction_type == "income": 
      description = f"Got {transaction.amount} from {transaction.description} on {transaction.date} into {account.bank_name}."
    else:
      description = f"Spent {transaction.amount} on {transaction.description} on {transaction.date} from {account.bank_name}.."
  ...
  filename =  results_dir / f"eval_rich_{datetime.now().strftime('%Y-%m-%d')}.json"
